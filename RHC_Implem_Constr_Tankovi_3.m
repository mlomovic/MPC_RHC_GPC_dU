...
% Ogranicenja
% Gornja i donja granica upravljanja
u_max = [1; 1];
u_min = [-0;-0];
% Gornja i donja granica promene upravljanja 
deltau_max = [0.5; 0.5];
deltau_min = [-0.5; -0.5];
...
r1=zeros(N_sim,1);%*0.7;
%r1(50:150,1)=1;
r2=zeros(N_sim,1)*0.5;
%r2(250:350,1)=1;
r1(50:150,1)=0.7;
r2(50:150,1)=0.5;
r1(250:350,1)=0.5;
...
%end;
for i = 1:Nc
    A_cons_tmp(1:Nc,1:Nc,i) = eye(Nc);
    A_cons_tmp(Nc+1:2*Nc,1:Nc,i) = -eye(Nc);
    
    A_cons_tmp(2*Nc+1:3*Nc,1:Nc,i) = tril(ones(Nc,Nc));
    A_cons_tmp(3*Nc+1:4*Nc,1:Nc,i) = -tril(ones(Nc,Nc));
end
for i = 1:n_in
    A_cons(i:n_in:4*n_in*Nc,i:n_in:Nc*n_in) = A_cons_tmp(:,:,i);
end
...
%figure
s(4) = subplot(3,2,[2,4])
plot(lik_stanja(:,1),lik_stanja(:,2)),grid
hold on
plot(lik_stanja(1,1),lik_stanja(1,1),'r*',lik_stanja(length(lik_stanja),1),lik_stanja(length(lik_stanja),2),'go')
xlabel('x_1')
ylabel('x_2')
legend('lik stanja','pocetno stanje','krajnje stanje')
